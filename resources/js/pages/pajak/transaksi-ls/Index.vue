...
const fields = [
    // { key: 'nomor_sp2d', label: 'Nomor SP2D', type: 'string', sortable: true, width: '180px' },
    {
...
async function loadData(page: number | string = 1) {
    ...
    try {
        ...
        const res = await axios.get(route('pajak.ls.data'), {
            params: {
...
function handleEdit(id: number) {
    ...
    if (item) {
        ...
        // Ensure selectedAkun is populated correctly to show the "Jenis Pajak" label
        ...
        if (foundAkun) {
            ...
            // Fallback to data already present in the item if masters aren't loaded yet
            selectedAkun.value = {
...
async function handleSubmit() {
    ...
    try {
        ...
        if (isEdit.value && editingId.value) {
            await axios.put(route('pajak.ls.update', editingId.value), payload);
        } else {
            await axios.post(route('pajak.ls.store'), payload);
        }
...
async function confirmDelete() {
    ...
    try {
        await axios.delete(route('pajak.ls.destroy', itemToDeleteId.value));
...
async function confirmSync() {
    ...
    try {
        ...
        toast.success('Data BKU LS & GU Berhasil Disinkronkan');
        loadData(1);
    } catch (error: any) {
...
function handleExport() {
    ...
    const url = route('pajak.ls.export') + '?' + params.toString();
...
<template>
    <Head title="Transaksi Pajak LS" />
    
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="px-4 py-6 space-y-6 flex-1 min-w-0 overflow-hidden">
            <HeadingSmall title="Pelaporan LS" description="Kelola transaksi belanja langsung per periode." />
                ...
                <div v-if="selectedMonth" class="relative min-h-[400px] flex-1 p-4 rounded-xl border border-sidebar-border/70 dark:border-sidebar-border md:min-h-min overflow-hidden bg-card">
                    <DataTable v-if="!isLoading"
                        ...
                        :buttonDinamis="[
                            ...
                            { label: 'Tambah Transaksi', variant: 'primary', onClick: 'handleCreate' }
                        ] as const"
                ...
                <div v-else class="flex flex-col items-center justify-center py-24 px-4 text-center bg-muted/20 rounded-2xl border-2 border-dashed border-sidebar-border/50 transition-all duration-300">
                    ...
                    <p class="text-muted-foreground max-w-xs mt-2">
                        Silakan pilih salah satu periode bulan di atas untuk mengelola rincian transaksi laporan pajak LS.
                    </p>
                </div>
            </div>
        ...
        <Dialog v-model:open="isModalOpen">
            <DialogContent class="sm:max-w-[700px]">
                <DialogHeader>
                    <DialogTitle>{{ isEdit ? 'Edit' : 'Tambah' }} Transaksi LS</DialogTitle>
                    <DialogDescription>Isi detail transaksi pajak di bawah ini.</DialogDescription>
                </DialogHeader>
                
                <div class="grid grid-cols-2 gap-4 py-4">
                    <div class="grid gap-2 col-span-2">
                        <Label for="nomor_sp2d">Nomor SP2D</Label>
                        <Input id="nomor_sp2d" v-model="form.nomor_sp2d" />
                        <InputError :message="errors.nomor_sp2d?.[0]" />
                    </div>
                    <div class="grid gap-2 col-span-2">
                        <Label for="nomor_dokumen">Nomor Dokumen</Label>
                        <Input id="nomor_dokumen" v-model="form.nomor_dokumen" />
                        <InputError :message="errors.nomor_dokumen?.[0]" />
                    </div>
                    <div class="grid gap-2 col-span-2">
                    ...
                    <div /> <!-- Spacer to force next item to new row -->
        ...
        <Dialog v-model:open="isSyncDialogOpen">
            <DialogContent class="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>Konfirmasi Sinkronisasi BKU (LS)</DialogTitle>
                    <DialogDescription>
                        Apakah Anda yakin ingin melakukan sinkronisasi data dari BKU ke Transaksi LS?
                    </DialogDescription>
                </DialogHeader>
