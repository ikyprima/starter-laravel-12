...
const fields = [
    { key: 'nama_skpd', label: 'SKPD Induk', type: 'string', sortable: false },
    { key: 'kode_skpd', label: 'Kode SKPD', type: 'string', sortable: true },
    { key: 'kode_sub_skpd', label: 'Kode Sub SKPD', type: 'string', sortable: true },
    { key: 'nama_sub_skpd', label: 'Nama Sub SKPD', type: 'string', sortable: true },
...
async function loadData() {
    try {
        ...
        const res = await axios.get(route('admin.sub-skpd.data'));
        dataResponse.value = res.data.data;
        subSkpds.value = res.data.data.data.map((item: any) => ({
            ...item,
            nama_skpd: item.skpd?.nama_skpd || '-'
        }));
    } catch (error) {
...
const buttonDinamis = [
    {
        label: "Tambah Sub SKPD",
        variant: "primary",
        onClick: "handleCreate"
    },
    {
        label: "Import Sub SKPD",
        variant: "outline",
...
function handleCreate() {
    router.visit(route('admin.sub-skpd.create'));
}

function handleEdit(id: number) {
    router.visit(route('admin.sub-skpd.edit', id));
}
...
function confirmDelete() {
    ...
    router.delete(route('admin.sub-skpd.destroy', deleteId.value), {
        onSuccess: () => {
...
async function startImport() {
    ...
    try {
        const res = await axios.get(route('admin.sub-skpd.import'));
        if (res.data.success) {
...
function handleSearch(search: string) {
    isLoading.value = true;
    axios.get(route('admin.sub-skpd.data'), { params: { search } })
        .then((res) => {
            dataResponse.value = res.data.data;
            subSkpds.value = res.data.data.data;
        })
            ...
            <Dialog v-model:open="isConfirmDelete">
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Apakah Anda yakin ingin menghapus data Sub SKPD ini? Tindakan ini tidak dapat dibatalkan.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
