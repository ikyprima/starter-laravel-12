...
async function syncBku(monthId: number) {
    ...
    try {
        const res = await axios.post(route('admin.skpd.bku.sync'), {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <AdminLayout>
            <div class="flex h-full flex-1 flex-col gap-4">
                <Card>
                    <CardHeader>
                        <CardTitle>Daftar BKU Per Bulan</CardTitle>
                        <CardDescription>
                                    ...
                                    <TableRow v-for="(month, index) in months" :key="month.id">
                                        ...
                                        <TableCell class="text-right flex justify-end gap-2">
                                            <Button 
                                                ...
                                                variant="secondary"
                                                size="sm"
                                                @click="router.visit(route('admin.skpd.bku.detail', { skpd: props.skpd?.id, bulan: month.id }))"
                                            >
                                                Detail
                                            </Button>
                                            <Button 
