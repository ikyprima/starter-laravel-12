...
async function syncBku(monthId: number) {
    syncingMonth.value = monthId;
    syncStatus.value[monthId] = { loading: true };
    
    try {
        const res = await axios.post(route('admin.skpd.bku-pajak.sync'), {
...
<template>
    <Head :title="'BKU Pajak - ' + skpd?.nama_skpd" />
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <AdminLayout>
            <div class="flex h-full flex-1 flex-col gap-4">
                <Card>
                    <CardHeader>
                        <CardTitle>Daftar BKU Pajak Per Bulan</CardTitle>
                        <CardDescription>
                                ...
                                <TableBody>
                                    <TableRow v-for="(month, index) in months" :key="month.id">
                                        ...
                                        <TableCell class="text-right flex justify-end gap-2">
                                            <Button 
                                                v-if="syncStatus[month.id]?.success"
                                                variant="secondary"
                                                size="sm"
                                                @click="router.visit(route('admin.skpd.bku-pajak.detail', { skpd: props.skpd?.id, bulan: month.id }))"
                                            >
                                                Detail
                                            </Button>
                                            <Button 
