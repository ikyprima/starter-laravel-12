...
const breadcrumbs = [
    ...
    { title: 'Realisasi Bulan', href: route('admin.skpd.realisasi', props.skpd?.id) },
    { title: 'Detail', href: '#' },
];
...
<template>
    <Head :title="'Detail Realisasi - ' + months[bulan || 0]" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <AdminLayout>
            <div class="flex h-full flex-1 flex-col gap-4 w-full min-w-0 overflow-hidden">
                <div class="flex items-center justify-between">
                    <Button variant="ghost" size="sm" @click="router.visit(route('admin.skpd.realisasi', props.skpd?.id))">
                ...
                <Card class="flex w-full flex-col min-w-0 overflow-hidden">
                    <CardHeader class="flex-none">
                        <CardTitle>Detail Realisasi {{ months[bulan || 0] }} {{ tahun }}</CardTitle>
                        <CardDescription>
                            ...
                            <Table>
                                <TableHeader class="sticky top-0 bg-background z-10">
                                    <TableRow>
                                        ...
                                        <TableHead class="text-xs">Sub Kegiatan</TableHead>
                                        <TableHead class="text-right text-xs">Nilai Realisasi</TableHead>
                                        <TableHead class="text-xs">Jenis</TableHead>
                                        <TableHead class="text-xs">Nomor SP2D</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    <TableRow v-for="item in (data as any[])" :key="item.id">
                                        <TableCell class="whitespace-nowrap text-xs">{{ formatDate(item.tanggal_dokumen) }}</TableCell>
                                        <TableCell>
                                                        ...
                                                        <Tooltip :delayDuration="200">
                                                            ...
                                                            <TooltipContent side="bottom" class="max-w-[400px] p-3 text-xs bg-popover text-popover-foreground shadow-md border">
                                                                <div class="flex flex-col gap-1">
                                                                    ...
                                                                    <p class="text-xs">{{ item.keterangan_dokumen }}</p>
                                                                </div>
                                                            </TooltipContent>
                                                        </Tooltip>
                                                    </TooltipProvider>
                                        ...
                                        <TableCell class="text-[10px] font-mono">{{ item.kode_sub_kegiatan }}</TableCell>
                                        <TableCell class="text-right text-xs whitespace-nowrap font-medium">{{ formatCurrency(item.nilai_realisasi) }}</TableCell>
                                        <TableCell>
                                            <div class="flex flex-col">
                                                <span class="text-[10px] font-bold">{{ item.jenis_dokumen }}</span>
                                                <span class="text-[10px] text-muted-foreground">{{ item.jenis_transaksi }}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell class="text-[10px] font-mono">{{ item.nomor_sp2d || '-' }}</TableCell>
                                    </TableRow>
