...
const fields = [
    { key: 'tanggal', label: 'Tanggal', type: 'string', sortable: true },
    { key: 'nomor_dokumen', label: 'Nomor Dokumen', type: 'string', sortable: true },
    { key: 'uraian', label: 'Uraian', type: 'string', sortable: false },
    { key: 'penerimaan', label: 'Penerimaan', type: 'amount', sortable: true },
    { key: 'pengeluaran', label: 'Pengeluaran', type: 'amount', sortable: true },
    { key: 'saldo', label: 'Saldo', type: 'amount', sortable: true },
    { key: 'ntpn', label: 'NTPN', type: 'string', sortable: true },
];

async function loadData() {
    try {
        isLoading.value = true;
        const res = await axios.get(route('admin.bku-pajak.data'));
...
async function handleSync() {
    try {
        isLoading.value = true;
        const res = await axios.post(route('admin.bku-pajak.sync'));
...
async function handleImport() {
    // This is a placeholder as actual import would need file upload
    try {
        isLoading.value = true;
        const res = await axios.post(route('admin.bku-pajak.import'));
        alert(res.data.message);
        loadData();
    } catch (error) {
...
function handleSearch(search: string) {
    isLoading.value = true;
    axios.get(route('admin.bku-pajak.data'), { params: { search } })
        .then((res) => {
            dataResponse.value = res.data.data;
            items.value = res.data.data.data;
        })
